...
public class BinaryTree<K extends Comparable<K>, V> implements IBinaryTree<K, V> {
    ...
    @Override
    public V min() throws Exception {
        Node n = getMinimum(root);
        return n.value;
    }
    ...
    private void removeNodeWithChild(Node node) {
        Node successor = (hasRightChild(node)) ? getMinimum(node.right) : getMaximum(node.left);
    ...
    private IAbstrDoubleList<V> sort() {
        IAbstrDoubleList<V> list = new AbstrDoubleList<>();
        return list;
